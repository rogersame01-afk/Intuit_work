...
#e2e
IDPS_END_POINT = "vkm.ps.idps.a.intuit.com" #e2e: vkm-e2e.ps.idps.a.intuit.com"
...
def _call_dnc_api(phone_entries, api_key, max_retries=3, backoff_base=5):
    """
    ...
    for batch_idx in range(batches):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                resp = requests.post(DNC_URL, headers=headers, json=payload, timeout=60)
                resp.raise_for_status()
                snippet = resp.text[:500] if resp.text else ''
                print(f"✅ DNC batch {batch_idx+1}/{batches} size={len(batch)} "
                      f"status={resp.status_code} attempt={attempt} snippet={snippet}")
                ...
                snippet = e.response.text[:500] if e.response is not None and e.response.text else ''
                print(f"⚠️  DNC batch {batch_idx+1}/{batches} attempt {attempt}/{max_retries} "
                      f"status={status} error={e} snippet={snippet}")
                if status is not None and status < 500:
                    raise  # 4xx = bad auth/payload, don't retry
                last_exc = e
        ...
        if last_exc is not None:
            raise last_exc  # all retries exhausted for this batch
...
def filter_out_dnc_numbers(df):
    ...
    try:
        ...
        raise  # <-- re-raise instead of exit(), this actually stops execution

    print("DNC API call successful; continuing with processing.")
...
df = spark.sql("""
  SELECT DISTINCT 
        ...
        auth_id AS UserId, -- changed this from pseudoonym_id to auth id on 2.4.25 for iep per matt m
    ...
    FROM  
    cgan_ustax_ws.efile_rejects_epc_ty25_cjo_final_concentrix_days_2_and_9 
...
pd_df = filter_out_dnc_numbers(pd_df)
pd_df.head()
...
try:
    ...
    content = pd_df.to_csv(index=False, escapechar='\\')
...
csv_string = pd_df.to_csv(index=False, escapechar='\\')  # Convert DataFrame to CSV
...
#######################################################################################################
# Define default campaign start time: 5:00 AM PST today
default_start_pst = PACIFIC.localize(datetime.combine(now_pst.date(), time(5, 0)))

# If current PST time is on or after 5 AM, use current time + 30 minutes
...
if weekday == 6:
    ...
    campaign_end_naive = datetime.combine(current_utc_date, time(20, 0, 0))  # 8 PM PST (Same day) changed to 5pm april 21
...
outbound_queue = "cg-us_ps_fed_efile_reject_campaign" #"cg_jshen2_test_322"
#e2e value cg-us_ps_fed_efile_reject_campaign-sys
#######################################################################################################
...
payload = {
    'data': {
        ...
        "dialerType": "Progressive",  # other options: "Agentless", "Predictive", "Progressive"
        "outboundQueue":  outbound_queue,  # "us_ps_fs_efile_reject_campaign queue", # "cg_jshen2_test_322" 
        "agentBandwidthAllocation": 95, #matt said set to 96 on feb 12 and 98 on feb 20 and 100 on 2/22 and 92 on 326, 90 on 513 and changed from predictive to progressive 95 from 515
        "filters": [],
...
csv_string = pd_df.to_csv(index=False, escapechar='\\')
...
if current_day >= 6:
    print("🚫 Today is a weekend (Sunday). Skipping campaign creation.")
else:
    # Call the API using the MultipartEncoder
    response = requests.request("POST", url, headers=headers, data=multipart_data)

    # ✅ Always print the raw Dialer API response info
    print(f"🔍 Dialer API Response — Status Code: {response.status_code}, Response Text: {response.text}")
