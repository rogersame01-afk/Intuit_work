DROP TABLE IF EXISTS cgan_ustax_ws.saves_cch_001_push_stg;
CREATE TABLE  cgan_ustax_ws.saves_cch_001_push_stg AS
WITH BASE_PUSH AS (
 SELECT *,
       COALESCE(s.canvas_name, s.campaign_name) AS outreach_campaign_name
    FROM tax_src.src_braze_turbotax_pushnotification_send S
    WHERE COALESCE(s.canvas_name, s.campaign_name)  like '%CSO%'
    AND year IN (2025, 2026))
  ,
  BASE_PUSH_FILTERED AS (
    SELECT 
    *,
    CASE 
...
-- Only pull sends beyond the watermark
 BASE_PUSH_DEPUDED AS (
SELECT *
FROM (
    SELECT
        id,
...
, push_engagement AS (
    SELECT
        ...
        -- Delivered = NOT bounced (push convention)
        , MAX(CASE WHEN b.external_user_id IS NULL
    ...
    FROM BASE_PUSH_DEPUDED AS s

    LEFT JOIN tax_src.src_braze_turbotax_pushnotification_open AS o
    ...
    LEFT JOIN tax_src.src_braze_turbotax_pushnotification_bounce AS b
...
-- Step 1: Check BASE_SMS_DEDUPLE - should be your baseline
 

-- Step 2: Check after sms_engagement GROUP BY


-- Step 3: Check after the final Smartsheet join

, push_final AS (
    SELECT
          'push'                                             AS outreach_channel
        ...
        , 'device_platform'                                 AS customer_contact_pii_type
        , base.platform                                     AS customer_contact_pii_value
...
INSERT INTO cgan_ustax_ws.saves_cch_001_push
SELECT *, CURRENT_DATE AS last_ingested_date
FROM cgan_ustax_ws.saves_cch_001_push_stg AS stg
WHERE NOT EXISTS (
    SELECT 1
    FROM cgan_ustax_ws.saves_cch_001_push AS tgt
