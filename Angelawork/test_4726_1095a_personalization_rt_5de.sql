...
DROP TABLE IF EXISTS cgan_general_published.ty25_4726_rt_5de_users;
CREATE TABLE cgan_general_published.ty25_4726_rt_5de_users as 
with test as (
    SELECT IXP.experiment_name as test_name
      , treatment_name AS recipe_name
      , ixp.id as auth_id
      , ixp.first_timestamp as first_assignment_timestamp
    FROM ixp_dwh.ixp_first_assignment ixp 
    WHERE 1=1
      AND ixp.experiment_id = 291057
      AND ixp.treatment_name IS NOT NULL
      -- AND ixp.version >= 4
      -- AND ixp.partitiondate >= DATE('2026-02-26')
),
...
select t.recipe_name as recipe
    ...
    , case when a.first_efile_reject_timestamp is not null then 1 else 0 end as efile_reject_flag
...
where mp.auth_id is null or (mp.rt_attach_flag = 1 or mp.fde_attach_flag = 1) -- rt and 5de only, for unit, reject and revenue counts;
;


DROP TABLE IF EXISTS cgan_general_published.ty25_4726_rt_5de_users_auth_level;
CREATE TABLE cgan_general_published.ty25_4726_rt_5de_users_auth_level as 
    select
          auth_id                                        
        , test_name                                                                     as experiment_id
        , date(first_assignment_timestamp)                                              as cohort_date
        , recipe                                                                        as condition 
        , 'Reject Rate'                                                                 as metric
        , 1.0 * ((count(distinct case when efile_reject_flag = 1 then auth_id end)) 
            / count(distinct auth_id))                                                  as outcome
    from cgan_general_published.ty25_4726_rt_5de_users
    group by 1,2,3,4,5
...
DROP TABLE IF EXISTS cgan_general_published.ty25_4726_rt_5de;
CREATE TABLE cgan_general_published.ty25_4726_rt_5de as 
select 
          experiment_id
        , cohort_date
        , condition
        , metric
        , outcome        
from cgan_general_published.ty25_4726_rt_5de_users_auth_level
;
