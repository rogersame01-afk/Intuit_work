...
DROP TABLE IF EXISTS cgan_ustax_ws.ty25_cg_experimentation_data_224297_v1;
CREATE TABLE cgan_ustax_ws.ty25_cg_experimentation_data_224297_v1 AS
    (
        WITH input_data AS
        (
    ...
    -- adding logic to limit to users who actually saw the experience
    ---------------------------------------------------------------------------------
    exp as (
        select
            pseudonym_id,
            max(case when experiment_ids LIKE '%224297%' then 1 end) as cs_experiment_flag,
            max(case when experiment_ids LIKE '%224297%497361%' then 1 else 0 end) as recipe_a_flag,
            max(case when experiment_ids LIKE '%224297%497362%' then 1 else 0  end) as recipe_b_flag,
            max(date(cast(date_time as timestamp))) as first_assignment_timestamp
        from tax_src.fact_clickstream_psd c
        where 1=1
            and tax_year = '2024'
            and load_date >= '2025060300'
            and load_date < '2025103100'
            and experiment_ids like '%224297%'
        group by 1
    ),

    gateway_screen as (
        select cs.pseudonym_id
            , min(from_unixtime(cs.event_ts)) as min_gateway_event_ts
        from tax_src.fact_clickstream_psd cs
        where cs.tax_year = '2024'
            and cs.load_date > '2025060300' -- test start date;
            and cs.screen = 'ippin-gateway-views-0'
            and cs.ui_object_detail = 'ippin-gateway-views-0-gatewayActions-affirmativeAction-action_YesAction'
            and cs.pseudonym_id is not null
        group by 1
    ),
    ---------------------------------------------------------------------------------
    ...
    test as (
            ...
            FROM
                ...
                inner join exp
                on pam.pseudonym_id = exp.pseudonym_id
                inner join gateway_screen gs
                on pam.pseudonym_id = gs.pseudonym_id
            WHERE
